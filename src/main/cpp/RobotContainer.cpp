...
void RobotContainer::SetDefaultCommands()
{
    m_drive.SetDefaultCommand(frc2::RunCommand(
        [this] {
            ...
            auto x = Deadzone(m_primaryController.GetY(frc::GenericHID::kLeftHand) * -1.0, OIConstants::kDeadzoneX);
            auto y = Deadzone(m_primaryController.GetX(frc::GenericHID::kLeftHand) * -1.0, OIConstants::kDeadzoneY);
...
void RobotContainer::ConfigureButtonBindings()
{
    ...
    /// B: Stop intake and cycle
    frc2::JoystickButton(&m_secondaryController, (int)frc::XboxController::Button::kB).WhenHeld(
        IntakeRelease(&m_intake)
    );

    /// Start: Reset Turret
    frc2::JoystickButton(&m_secondaryController, (int)frc::XboxController::Button::kStart).WhenPressed(
